...
\emph{Unit of analysis}: we study dataset-level statistics rather than
per-sample certificates.
\emph{Timing}: we operate before training, with no trained model.
\emph{Output}: a deployable scalar risk score, not a post-hoc
memorization label.
...
None of these works ask how the \emph{benefit} of a given $\varepsilon$
