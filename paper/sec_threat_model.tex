...
A \emph{membership inference attack} (MIA) is an algorithm
...
The \emph{MIA advantage} of $\mathcal{A}$ is:
\begin{equation}
  ...
  \label{eq:adv}
\end{equation}
We denote the \emph{dataset-level} risk as the expected advantage over samples:
\begin{equation}
...
We consider two principals: a \emph{data holder} who trains a model on $D$
and may later publish it, and an \emph{adversary} who wishes to infer
membership in $D$.
...
The adversary has \emph{black-box} access to $f_\theta$: it may submit
...
white-box access; results are consistent.

\paragraph{Adversary knowledge.}
The adversary knows the \emph{data domain distribution} $\mathcal{P}$
(e.g., it knows the dataset came from medical records of a particular
demographic) but does \emph{not} know the exact set $D$.
...
Before training begins, the data holder computes a \emph{Dataset Privacy
Risk Index} (DPRI) from the raw dataset $D$ using only data-distributional
statistics---no trained model required.
...
\paragraph{Dual use.}
DPRI can also be used \emph{offensively}: an adversary who has access to
candidate target datasets (e.g., a public benchmark or a leaked schema)
can compute DPRI \emph{before} mounting an attack and prioritize
