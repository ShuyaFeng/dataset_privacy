...
For the \emph{in-distribution} and \emph{out-distribution} of a trained model
$f_\theta$, define the \emph{score distributions}:
\begin{equation}
...
At the \emph{dataset level}, the aggregate advantage satisfies:
\begin{equation}
  \mathrm{Risk}(D)
  \;\leq\;
...
\emph{far from their neighbours} ($u(x)$ large, i.e.\ isolated)
and when the \emph{local population is sparse} ($\rho(x)$ small).
Both quantities are computable directly from $D$---no model training required.
...
\emph{Step 2: KL in terms of loss gap.}
...
\emph{Step 3: Density correction.}
...
This means the bound explains why dataset structure is the \emph{dominant}
