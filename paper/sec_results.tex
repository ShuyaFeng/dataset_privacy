...
\subsection{Finding 1: Dataset, Model, and Attack Each Matter}
\label{sec:finding1}

We decompose the variation in MIA AUC by the spread of group-mean AUC across
each factor. Dataset choice accounts for $36.4\%$ of the variation, model
architecture for $39.4\%$, and the attack method for $24.2\%$.

The headline is that all three matter, and to a comparable degree. Dataset
structure is not a fixed backdrop to be ignored: it moves risk as much as
the choice of model and more than the choice of attack. This challenges the
common practice of reporting a single attack number on a single dataset, as
if the dataset were incidental. It conflates three distinct sources of
variation. We note that the large model effect is driven mainly by the
Random Forest, whose unbounded depth makes it memorize and leak far more
than the regularized alternatives.
