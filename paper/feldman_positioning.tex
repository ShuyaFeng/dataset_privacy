...
%
Feldman~\cite{feldman2020memorization} showed that individual rare
...
dataset-level structural statistics predict aggregate
...
Unit of analysis: we study dataset-level statistics rather
than per-sample certificates.
Timing: we operate before training, producing a risk estimate
without a trained model.
Output: we produce a deployable scalar index (DPRI) that
practitioners can compute at data-collection time, not a post-hoc
explanation of memorization.
Concretely, our experiments include datasets where no individual
